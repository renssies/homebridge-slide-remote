...
SlidePlatform.prototype.accessories = function(callback) {
    ...
    if (this.accessoryList.length <= 0) {
        ...
        this.getSlides().then((slides) => {
        	...
        	// Workaround: The accessories callback does not support a error. 
        	// For now we return the accessory list (which is empty).
        	this.log.warn("Failed to get slides on your account. The slide API might be unreachable or down, error: " + error)
            callback(this.accessoryList)
        })
    } else {
...
SlidePlatform.prototype.request = function(method, endpoint, parameters, accessToken) {
