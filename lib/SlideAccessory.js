...
function SlideAccessory(platform, data) {
    ...
    let pollInterval = platform.configJSON["poll_interval"] || 300 
    poll(this.updateSlideInfo.bind(this), pollInterval * 1000)
}

/* (Periodic) update functions */

SlideAccessory.prototype.updateSlideInfo = function(bypassCache = false) {
...
SlideAccessory.prototype.getSlideInfo = function(bypassCache = false) {
    return this.platform.request("GET", "slide/" + this.identifier + "/info", null, this.platform.accessToken, bypassCache)
}
...
SlideAccessory.prototype.getCurrentPosition = function(callback) {
    this.getSlideInfo(true).then((device_info) => {
        if (!device_info.data) {
            callback(Error("Invalid response"))
            return
        }
